...
const PostLetter = () => {
    ...
    return (<Container>
        <Button animated="fade" size="huge" basic onClick={() => setModalOpen(true)}>
            <Button.Content style={{ opacity: modalOpen && 0 }} visible>Dear future you</Button.Content>
            <Button.Content style={{ opacity: modalOpen && 0 }} hidden >
                <Icon name="mail" />
            </Button.Content>
        </Button>
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} size="large" dimmer="blurring" >
            <Form onSubmit={(e) => handleSubmit(e)} size="huge"  >
                ...
                />
                <Form.Button content="See you letter again" disabled={!letter} />
            </Form>
            <Modal open={nestedModalOpen} onClose={() => HandlenestedModalClose()}>
